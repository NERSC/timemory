...
foreach(_VAR FIND_STRATEGY FIND_VIRTUALENV FIND_FRAMEWORK FIND_IMPLEMENTATIONS
    ...
    if(DEFINED Python3_${_VAR})
        set(Python_${_VAR}
            "${Python3_${_VAR}}"
            CACHE STRING "Set via Python3_${_VAR} setting (timemory)")
...
set(Python_ADDITIONAL_VERSIONS
    "3.9;3.8;3.7;3.6"
    CACHE STRING "Python versions supported by timemory")

# override types to search for
if(TIMEMORY_PYTHON_VERSION)
    set(Python_ADDITIONAL_VERSIONS
        ${TIMEMORY_PYTHON_VERSION}
        CACHE STRING "Python versions supported by timemory" FORCE)
...
if(_PYVERSION_LAST AND (TIMEMORY_PYTHON_VERSION VERSION_LESS _PYVERSION_LAST
...
set(PYTHON_LIBRARY_DEBUG
    ...
    CACHE FILEPATH "Set via Python3_LIBRARY_DEBUG (timemory)" FORCE)
...
if(Python3_LIBRARY_RELEASE)
    set(PYTHON_LIBRARY
        "${Python3_LIBRARY_RELEASE}"
        CACHE FILEPATH "Set via Python3_LIBRARY (timemory)" FORCE)
    set(PYTHON_LIBRARIES
        "${Python3_LIBRARY_RELEASE}"
        CACHE FILEPATH "Set via Python3_LIBRARIES (timemory)" FORCE)
...
set(PYTHON_LIBRARY_DIRS
    ...
    CACHE PATH "Set via Python3_LIBRARY_DIRS (timemory)" FORCE)
set(PYTHON_LINK_OPTIONS
    "${Python3_LINK_OPTIONS}"
    CACHE STRING "Set via Python3_LINK_OPTIONS (timemory)" FORCE)

# module
set(PYTHON_MODULE_EXTENSION
    "${Python3_MODULE_EXTENSION}"
    CACHE STRING "Set via Python3_MODULE_EXTENSION (timemory)" FORCE)
set(PYTHON_MODULE_PREFIX
    "${Python3_MODULE_PREFIX}"
    CACHE STRING "Set via Python3_MODULE_PREFIX (timemory)" FORCE)

# version
set(PYTHON_VERSION
    "${Python3_VERSION}"
    CACHE STRING "Set via Python3_VERSION (timemory)" FORCE)
set(PYTHON_VERSION_MAJOR
    "${Python3_VERSION_MAJOR}"
    CACHE STRING "Set via Python3_VERSION_MAJOR (timemory)" FORCE)
set(PYTHON_VERSION_MINOR
    "${Python3_VERSION_MINOR}"
    CACHE STRING "Set via Python3_VERSION_MINOR (timemory)" FORCE)
...
if(NOT "${TIMEMORY_PYTHON_VERSION}" MATCHES "${PYBIND11_PYTHON_VERSION}*")
    ...
    message(
        FATAL_ERROR "Mismatched 'TIMEMORY_PYTHON_VERSION' and 'PYBIND11_PYTHON_VERSION'")
endif()
...
elseif(SPACK_BUILD OR "${TIMEMORY_INSTALL_PYTHON}" STREQUAL "lib")
    ...
    if(ERR_CODE)
        if("${TIMEMORY_INSTALL_PYTHON}" STREQUAL "global")
            message(
                FATAL_ERROR
                    "timemory could not install python files to ${Python3_SITEARCH} (not writable):\n${ERR_MSG}"
                )
        endif()
...
if(APPLE)
    if(CMAKE_VERSION VERSION_LESS 3.18)
        ...
        target_link_libraries(
            timemory-python INTERFACE "$<$<LINK_LANGUAGE:CXX>:-undefined dynamic_lookup>")
    endif()
endif()
...
if(WIN32)
    ...
    # in cmake code at
    # C:/projects/timemory/build-timemory/tests/test-python-install-import.cmake:3 when
