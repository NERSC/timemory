...
# The FPHSA helper provides standard way of reporting final search results to the user
# including the version and component checks.
...
function(_boost_get_existing_target component target_var)
    ...
    if(component MATCHES "^([a-z_]*)(python|numpy)([1-9])\\.?([0-9])?$")
        ...
        # etc.
        list(
            APPEND names "${CMAKE_MATCH_1}${CMAKE_MATCH_2}" # python
            "${CMAKE_MATCH_1}${CMAKE_MATCH_2}${CMAKE_MATCH_3}" # pythonX
            "${CMAKE_MATCH_1}${CMAKE_MATCH_2}${CMAKE_MATCH_3}${CMAKE_MATCH_4}" # pythonXY
            )
    endif()
    # https://github.com/boost-cmake/boost-cmake uses boost::file_system etc. So handle
    # similar constructions of target names
    ...
    foreach(prefix Boost boost)
        foreach(name IN LISTS names)
            if(TARGET "${prefix}::${name}")
                ...
                # target for compatibility.  Unwrap this layer so we can extract real
                # info.
...
function(_boost_get_canonical_target_name component target_var)
    ...
    if(component MATCHES "^([a-z_]*)(python|numpy)([1-9])\\.?([0-9])?$")
        ...
        # etc.
        set(${target_var}
            "Boost::${CMAKE_MATCH_1}${CMAKE_MATCH_2}"
            PARENT_SCOPE)
    else()
        set(${target_var}
            "Boost::${component}"
        ...
        if(Boost_${comp}_FOUND)
            ...
            if(NOT TARGET)
                if(Boost_DEBUG OR Boost_VERBOSE)
                    message(
                        WARNING
                            "Could not find imported target for required component '${comp}'. Legacy variables for this component might be missing. Refer to the documentation of your Boost installation for help on variables to use."
                        )
                endif()
                continue()
            endif()
            ...
            _boost_set_if_unset(
                Boost_${uppercomp}_LIBRARIES "${target}") # Very old legacy variable
            ...
            elseif(Boost_DEBUG OR Boost_VERBOSE)
                # For projects using only the Boost::* targets this warning can be safely
                # ignored.
                if(NOT Boost_FIND_QUIETLY)
                    message(
                        WARNING
                            "Imported target '${target}' for required component '${comp}' has no artifact. Legacy variables for this component might be missing. Refer to the documentation of your Boost installation for help on variables to use."
                        )
                endif()
            endif()
...
# -------------------------------------------------------------------------------
# Before we go searching, check whether a boost cmake package is available, unless the
# user specifically asked NOT to search for one.
#
...
if(NOT Boost_NO_BOOST_CMAKE)
    ...
    if(Boost_FOUND)
        # Convert component found variables to standard variables if required Necessary
        # for legacy boost-cmake and 1.70 builtin BoostConfig
...
#
# Print debug text if Boost_DEBUG is set. Call example:
# _Boost_DEBUG_PRINT("${CMAKE_CURRENT_LIST_FILE}" "${CMAKE_CURRENT_LIST_LINE}" "debug
# message")
#
...
#
# _Boost_DEBUG_PRINT_VAR(file line variable_name [ENVIRONMENT] [SOURCE "short explanation
# of origin of var value"])
#
# ENVIRONMENT - look up environment variable instead of CMake variable
#
# Print variable name and its value if Boost_DEBUG is set. Call example:
...
function(_Boost_GUESS_COMPILER_PREFIX _ret)
    ...
    elseif("x${CMAKE_CXX_COMPILER_ID}" STREQUAL "xMSVC" OR "x${CMAKE_CXX_SIMULATE_ID}"
                                                           STREQUAL "xMSVC")
...
function(_Boost_COMPONENT_DEPENDENCIES component _ret)
    ...
    #   elseif(NOT Boost_VERSION_STRING VERSION_LESS \1\.\2\.\3 AND Boost_VERSION_STRING
    #   VERSION_LESS xxxx);
    #
    ...
    # was the latest at the time of writing, making 1.60 the next. Identical consecutive
    # releases were then merged together by updating the end range of the first block and
    # removing the following redundant blocks.
    #
    ...
    if(Boost_VERSION_STRING)
        if(Boost_VERSION_STRING VERSION_LESS 1.33.0)
            if(NOT Boost_FIND_QUIETLY)
                message(
                    WARNING
                        "Imported targets and dependency information not available for Boost version ${Boost_VERSION_STRING} (all versions older than 1.33)"
                    )
            endif()
        ...
        elseif(Boost_VERSION_STRING VERSION_LESS 1.45.0)
            ...
            set(_Boost_IOSTREAMS_DEPENDENCIES regex)
            set(_Boost_MATH_DEPENDENCIES math_c99 math_c99f math_c99l math_tr1 math_tr1f
                                         math_tr1l random serialization)
            set(_Boost_MPI_DEPENDENCIES serialization)
            set(_Boost_MPI_PYTHON_DEPENDENCIES python${component_python_version} mpi
                                               serialization)
            set(_Boost_THREAD_DEPENDENCIES date_time)
        ...
        elseif(Boost_VERSION_STRING VERSION_LESS 1.55.0)
            ...
            set(_Boost_FILESYSTEM_DEPENDENCIES system)
            set(_Boost_IOSTREAMS_DEPENDENCIES regex)
            set(_Boost_LOG_DEPENDENCIES log_setup date_time system filesystem thread
                                        regex chrono)
            set(_Boost_MATH_DEPENDENCIES math_c99 math_c99f math_c99l math_tr1 math_tr1f
                                         math_tr1l regex random)
            set(_Boost_MPI_DEPENDENCIES serialization)
        ...
        elseif(Boost_VERSION_STRING VERSION_LESS 1.70.0)
            ...
            set(_Boost_IOSTREAMS_DEPENDENCIES regex)
            set(_Boost_LOG_DEPENDENCIES date_time log_setup filesystem thread regex
                                        chrono atomic)
            set(_Boost_MATH_DEPENDENCIES math_c99 math_c99f math_c99l math_tr1 math_tr1f
                                         math_tr1l atomic)
            set(_Boost_MPI_DEPENDENCIES serialization)
...
#
# TODO (Re)Check compiler features on (every?) release ??? One may use the following
# command to get the files to check:
#
...
function(_Boost_COMPILER_FEATURES component _ret)
    ...
    if(NOT Boost_VERSION_STRING VERSION_LESS 1.67.0)
        ...
        set(_Boost_CONTRACT_COMPILER_FEATURES cxx_lambdas cxx_variadic_templates)
    endif()
...
if(Boost_INCLUDE_DIR)
    ...
    if(NOT CMAKE_VERSION VERSION_LESS 3.15)
        ...
        cmake_policy(
            GET CMP0093 _Boost_CMP0093 PARENT_SCOPE # undocumented, do not use outside of
                                                    # CMake
            )
...
if(DEFINED Boost_ARCHITECTURE)
    ...
    if(NOT "x${CMAKE_CXX_COMPILER_ARCHITECTURE_ID}" STREQUAL "x"
       AND NOT Boost_VERSION_STRING VERSION_LESS 1.66.0)
...
if(Boost_FOUND)
    # The builtin CMake package in Boost 1.70+ introduces a new name for the header-only
    # lib, let's provide the same UI in module mode
    if(NOT TARGET Boost::headers)
        add_library(Boost::headers INTERFACE IMPORTED)
    ...
    # Define the old target name for header-only libraries for backwards compat.
    if(NOT TARGET Boost::boost)
        add_library(Boost::boost INTERFACE IMPORTED)
    ...
    # contain any interface definitions on WIN32, but is created on all platforms to keep
    # end user code free from platform dependent code.  Also provide convenience targets
    # to disable autolinking and enable dynamic linking.
    ...
    if(WIN32)
        ...
        # libraries.  If you are linking to a dynamic runtime, then you can choose to link
        # to either a static or a dynamic Boost library, the default is to do a static
        ...
        # dynamically.  Alternatively you can force all Boost libraries to dynamic link by
        ...
        # BOOST_LIB_DIAGNOSTIC will cause the auto-linking code to emit a #pragma message
