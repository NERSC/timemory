...
foreach(_var ${_cached_vars})
    if(NOT "${_var}" IN_LIST EXCLUDED_VARIABLES)
        list(APPEND TIMEMORY_CMAKE_ARGS ${_var})
...
macro(SET_DEFAULT VAR)
    ...
    # passed to the timemory configuration
    if("${VAR}" IN_LIST TIMEMORY_CMAKE_ARGS)
        list(REMOVE_ITEM TIMEMORY_CMAKE_ARGS "${VAR}")
    endif()
endmacro()
...
SET_DEFAULT(BINARY_DIR      "${WORKING_DIR}/build-timemory/${CMAKE_HOST_SYSTEM_NAME}")  # build directory
...
if(NOT BUILD_TYPE)
    # default for timemory if not specified
...
foreach(_ARG ${TIMEMORY_CMAKE_ARGS})
    if(NOT "${${_ARG}}" STREQUAL "")
        ...
        if("${_ARG_TYPE}" STREQUAL "UNINITIALIZED")
            ...
            elseif(EXISTS "${${_ARG}}" AND (IS_DIRECTORY "${${_ARG}}" OR "${_ARG}" STREQUAL "CMAKE_INSTALL_PREFIX"))
                set(_ARG_TYPE "PATH")
...
set(CONFIG_ARGS "-C ${BINARY_REALDIR}/initial-cache.cmake ${CMAKE_ARGS}")


# generate the CTestConfig.cmake
configure_file(
    ${SOURCE_DIR}/cmake/Templates/CTestConfig.cmake.in
    ${BINARY_REALDIR}/CTestConfig.cmake
    @ONLY)

# copy/generate the dashboard script
configure_file(
    ${SOURCE_DIR}/cmake/Templates/timemory-ctest.cmake.in
    ${BINARY_REALDIR}/timemory-ctest.cmake
    @ONLY)
...
message(STATUS "Executing '${CTEST_COMMAND} -S timemory-ctest.cmake ${CTEST_ARGS}'...")
message(STATUS "")
...
execute_process(
    COMMAND             ${CTEST_COMMAND} -S timemory-ctest.cmake ${CTEST_ARGS}
    RESULT_VARIABLE     RET
    WORKING_DIRECTORY   ${BINARY_REALDIR}
    )
