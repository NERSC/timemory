...
def configure():
    ...
    parser.add_argument("--lto", help="TIMEMORY_BUILD_LTO=ON",
                        default=False, action='store_true')
    parser.add_argument("--ipo", help="CMAKE_INTERPROCEDURAL_OPTIMIZATION=ON",
                        default=False, action='store_true')
    ...
    parser.add_argument("--quick", help="Only run unit tests (not examples)",
                        default=False, action='store_true')
...
def run_pyctest():
    ...
    pyct.set("CTEST_CUSTOM_COVERAGE_EXCLUDE",
    ...
    def construct_command(cmd, args):
        ...
        if args.profile is not None:
            ...
            if args.profile == "cpu":
                ...
                for itr in ["alloc_objects", "alloc_space",
    ...
    if args.quick:
        ...
        pyct.test(construct_name("ex-cxx-minimal-library-overload"),
                  construct_command(
                      ["./ex_cxx_minimal_library_overload"], args),
                  {"WORKING_DIRECTORY": pyct.BINARY_DIRECTORY,
                   "LABELS": pyct.PROJECT_NAME,
                   "TIMEOUT": "300",
                   "ENVIRONMENT": test_env})
        ...
        if not args.python:
            ...
            pyct.test(construct_name("ex-cpu-roofline.sp"),
                      construct_roofline_command(
                          ["./ex_cpu_roofline.sp"], 'cpu-roofline.sp',
                          ['-t', 'cpu_roofline']),
                      {"WORKING_DIRECTORY": pyct.BINARY_DIRECTORY,
                       "LABELS": pyct.PROJECT_NAME,
                       "TIMEOUT": "900",
